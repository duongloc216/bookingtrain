    ...
    return (
      <div style={{ background: '#f7f7fa', minHeight: '100vh', width: '100vw', fontFamily: 'Montserrat, Arial, sans-serif' }}>
        ...
        <div style={{ background: '#1976d2', color: '#fff', padding: '24px 0 10px 0', textAlign: 'left', position: 'relative' }}>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', background: '#fff', borderRadius: '16px 16px 0 0', marginTop: 18 }}>
            {tabList.map(t => (
              <div
                key={t.id}
                onClick={() => setTab(t.id)}
                style={{ flex: 1, textAlign: 'center', fontWeight: 700, fontSize: 17, padding: '12px 0 10px 0', color: tab === t.id ? '#1976d2' : '#888', borderBottom: tab === t.id ? '3px solid #1976d2' : '3px solid transparent', cursor: 'pointer', transition: 'color 0.2s' }}
              >
                {t.label}
              </div>
            ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 64 }}>
          ...
          <div style={{ fontSize: 15, color: '#444', textAlign: 'center', marginBottom: 28, maxWidth: 320 }}>
            Log in to view your order history from the last 3 months
          </div>
          <button
        ...
        <div style={{ position: 'fixed', left: 0, bottom: 0, width: '100%', background: '#fff', borderTop: '1.5px solid #e0e0e0', display: 'flex', justifyContent: 'space-around', alignItems: 'center', height: 62, zIndex: 10 }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', color: '#1976d2', fontWeight: 700 }}><div style={{ fontSize: 22 }}>📅</div><div style={{ fontSize: 13 }}>My Tickets</div></div>
